...
function sp_uploadFile($files){
	...
	if(is_array($files['dlg-upload-file']['name'])){
...
$zip->setZipFile($dir.$return_file);
	...
	$filename = ''.sp_client_upload_filename($current_user->ID) .''.$_FILES['dlg-upload-file']['name'].'';
	$target_path = $dir .$filename; 
	
	move_uploaded_file($_FILES['dlg-upload-file']['tmp_name'], $target_path);
